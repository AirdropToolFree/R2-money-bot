...
// Hàm hỏi người dùng có muốn sử dụng proxy không
async function askProxyUsage() {
    return new Promise((resolve) => {
        readline.question('Bạn có muốn sử dụng proxy không? (y/n): ', (answer) => {
...
async function checkProxyIP(proxy) {
    if (!config.useProxy) {
        log("Chế độ không sử dụng proxy được bật", 'info');
...
async function addLiquidityR2USD_SR2USD(privateKey, networkConfig, r2usdAmount, sr2usdAmount) {
    ...
    try {
        ...
        log(`Số dư thực tế - R2USD: ${actualR2usdBalance.formattedBalance}, SR2USD: ${actualSr2usdBalance.formattedBalance}`, 'info');
        ...
        if (finalR2usdAmount < minAmount || finalSr2usdAmount < minAmount) {
            log(`Số dư không đủ tối thiểu: R2USD (${actualR2usdBalance.formattedBalance}) hoặc SR2USD (${actualSr2usdBalance.formattedBalance}) < 1`, 'warning');
            return false;
        }
        ...
        log(`Số lượng được điều chỉnh để thêm thanh khoản: ${ethers.formatUnits(finalR2usdAmount, 6)} R2USD và ${ethers.formatUnits(finalSr2usdAmount, 6)} SR2USD`, 'info');
        ...
        log(`Đang thêm thanh khoản R2USD/SR2USD: ${ethers.formatUnits(finalR2usdAmount, networkConfig.tokens.R2USD.decimals)} R2USD và ${ethers.formatUnits(finalSr2usdAmount, networkConfig.tokens.SR2USD.decimals)} SR2USD...`, 'custom');
...
async function checkAndClaimSeason0(privateKey, networkConfig, userAddress, token, proxy, userAgent) {
...
async function processNetworkTasks(privateKey, networkConfig, networkName, userAddress, token, proxy, userAgent) {
    ...
    if (networkName === 'Sepolia') {
        await checkAndClaimSeason0(privateKey, networkConfig, userAddress, token, proxy, userAgent);

        const r2TokenBalance = await checkTokenBalance(privateKey, networkConfig, 'R2_TOKEN');
        log(`Số dư R2 token: ${r2TokenBalance.formattedBalance}`, 'info');

        if (r2TokenBalance.balance > BigInt(100)) {
            log(`Số dư R2 token lớn hơn 100 (${r2TokenBalance.formattedBalance}). Đang thực hiện swap...`, 'custom');
        ...
        log(`Số dư USDC đủ. Đang đổi ${usdcBalanceFormatted} USDC (100%) sang R2USD...`, 'custom');
        ...
        log(`Đã đổi thành công ${usdcBalanceFormatted} USDC sang R2USD cho ${userAddress}`, 'success');

        const r2usdBalance = await checkTokenBalance(privateKey, networkConfig, 'R2USD');
        log(`Số dư R2USD sau swap: ${r2usdBalance.formattedBalance}`, 'info');

        const minR2usdAmount = ethers.parseUnits('1', networkConfig.tokens.R2USD.decimals);
        if (r2usdBalance.balance < minR2usdAmount) {
            log(`Số dư R2USD không đủ để tiếp tục (< 1 R2USD).`, 'warning');
            return;
        }
        ...
        log(`Đang stake ${r2usdToStakeFormatted} R2USD (50%) sang SR2USD...`, 'custom');
        ...
        log(`Đã stake thành công ${r2usdToStakeFormatted} R2USD sang SR2USD cho ${userAddress}`, 'success');

        const updatedR2usdBalance = await checkTokenBalance(privateKey, networkConfig, 'R2USD');
        const sr2usdBalance = await checkTokenBalance(privateKey, networkConfig, 'SR2USD');
        log(`Số dư R2USD (sau stake): ${updatedR2usdBalance.formattedBalance}`, 'info');
        ...
        if (sr2usdBalance.balance >= minR2usdAmount && updatedR2usdBalance.balance >= minR2usdAmount) {
            ...
            log(`Số dư SR2USD và R2USD đủ. Đang thêm thanh khoản với ${liquidityAmountFormatted} SR2USD và ${liquidityAmountFormatted} R2USD...`, 'custom');
            ...
            log(`Số dư SR2USD (${sr2usdBalance.formattedBalance}) hoặc R2USD (${updatedR2usdBalance.formattedBalance}) không đủ để thêm thanh khoản.`, 'warning');
        }
        ...
        log(`Số dư USDC đủ. Đang đổi ${usdcToSwapFormatted} USDC (98%) sang R2USD...`, 'custom');

        const r2usdContractAddress = networkConfig.tokens.R2USD.address;
        const usdcAddress = networkConfig.tokens.USDC.address;

        log(`Đang phê duyệt ${usdcToSwapFormatted} USDC cho contract ${r2usdContractAddress}...`, 'custom');
        ...
        log(`Đang đổi ${usdcToSwapFormatted} USDC sang R2USD trên contract ${r2usdContractAddress}...`, 'custom');
        try {
            const tx = await wallet.sendTransaction({
                to: r2usdContractAddress,
                data: data,
            ...
            log(`Giao dịch swap đã gửi: ${tx.hash}`, 'info');
            await tx.wait();
            log(`Swap ${usdcToSwapFormatted} USDC sang R2USD thành công`, 'success');
        } catch (error) {
            log(`Lỗi khi swap USDC sang R2USD: ${error.message}${error.reason ? ` (Reason: ${error.reason})` : ''}`, 'error');
            return;
        }
        ...
        if (r2usdBalance.balance >= minR2usdAmount) {
            const stakingContractAddress = networkConfig.stakingContracts.R2USD.address;
            log(`Đang phê duyệt ${r2usdBalance.formattedBalance} R2USD cho staking contract ${stakingContractAddress}...`, 'custom');
            const r2usdApproved = await approveToken(privateKey, networkConfig, r2usdContractAddress, stakingContractAddress, r2usdBalance.balance);
            if (!r2usdApproved) {
                log(`Phê duyệt R2USD thất bại, bỏ qua staking.`, 'warning');
                return;
            }
            ...
            log(`Đang stake ${r2usdBalance.formattedBalance} R2USD sang SR2USD trên contract ${stakingContractAddress}...`, 'custom');
            try {
                ...
                log(`Giao dịch stake đã gửi: ${stakeTx.hash}`, 'info');
                await stakeTx.wait();
                log(`Stake ${r2usdBalance.formattedBalance} R2USD sang SR2USD thành công`, 'success');
            } catch (error) {
                log(`Lỗi khi stake R2USD: ${error.message}${error.reason ? ` (Reason: ${error.reason})` : ''}`, 'error');
                return;
            }
        ...
        log(`Số dư USDC đủ. Đang đổi ${usdcToSwapFormatted} USDC (50%) sang R2USD...`, 'custom');
        ...
        if (r2usdBalance.balance >= minR2usdAmount && usdcBalanceCheck.balance >= minR2usdAmount) {
            ...
            log(`Đang phê duyệt ${ethers.formatUnits(usdcToAdd, networkConfig.tokens.USDC.decimals)} USDC cho contract ${liquidityContractAddress}...`, 'custom');
            ...
            log(`Đang phê duyệt ${ethers.formatUnits(r2usdToAdd, networkConfig.tokens.R2USD.decimals)} R2USD cho contract ${liquidityContractAddress}...`, 'custom');
            ...
            const deadline = Math.floor(Date.now() / 1000) + 60 * 20;
            log(`Đang thêm thanh khoản USDC/R2USD với ${ethers.formatUnits(usdcToAdd, networkConfig.tokens.USDC.decimals)} USDC và ${ethers.formatUnits(r2usdToAdd, networkConfig.tokens.R2USD.decimals)} R2USD...`, 'custom');
            try {
                const liquidityContract = new ethers.Contract(liquidityContractAddress, liquidityAbi, wallet);
            ...
            log(`Số dư R2USD (${r2usdBalance.formattedBalance}) hoặc USDC (${usdcBalanceCheck.formattedBalance}) không đủ để thêm thanh khoản.`, 'warning');
        }
    }
}
...
async function getUserPoints(token, userAddress, proxy, userAgent) {
...
async function loginApi(privateKey, proxy, userAgent) {
...
async function checkReferral(token, userAddress, proxy, userAgent) {
...
async function initiateTransaction(wallet, privateKey) {
...
async function bindReferral(token, userAddress, proxy, userAgent) {
...
async function main() {
    config.useProxy = await askProxyUsage();
    log(`Chế độ sử dụng proxy: ${config.useProxy ? 'BẬT' : 'TẮT'}`, 'info');
    ...
    try {
        ...
        if (config.useProxy) {
            try {
                ...
                if (privateKeys.length !== proxies.length || privateKeys.length !== userAgents.length) {
                    log(`Số lượng ví (${privateKeys.length}), proxy (${proxies.length}), và user agent (${userAgents.length}) không khớp.`, 'error');

                    // Tự động lặp lại proxy/user agent nếu cần
                    if (proxies.length > 0 && userAgents.length > 0) {
                        log(`Sử dụng proxy và user agent lặp lại để phù hợp với số lượng ví.`, 'warning');
                        ...
                        log(`Không đủ proxy/user agent, chuyển sang chế độ không sử dụng proxy.`, 'warning');
                        config.useProxy = false;
                    }
                }
            } catch (error) {
                log(`Lỗi khi đọc file proxy/user agent: ${error.message}`, 'error');
                log(`Chuyển sang chế độ không sử dụng proxy.`, 'warning');
                config.useProxy = false;
            }
        }

        // Nếu không dùng proxy, tạo mảng rỗng
        if (!config.useProxy) {
        ...
        log(`Đã đọc ${privateKeys.length} ví${config.useProxy ? `, ${proxies.length} proxy, và ${userAgents.length} user agent` : ''}`, 'info');

        if (privateKeys.length === 0) {
            log(`Không có ví hợp lệ để xử lý. Chương trình sẽ thoát.`, 'error');
            process.exit(1);
        }
    } catch (error) {
    ...
    for (let i = 0; i < privateKeys.length; i++) {
        ...
        try {
            ...
            const isBound = await checkReferral(token, userAddress, proxy, userAgent);
            ...
            if (!isBound) {
                ...
                const bound = await bindReferral(token, userAddress, proxy, userAgent);
            ...
            await getUserPoints(token, userAddress, proxy, userAgent);

            // Xử lý các network tasks
            await processNetworkTasks(privateKey, networkConfigs.sepolia, 'Sepolia', userAddress, token, proxy, userAgent);
            await sleep(Math.floor(Math.random() * 3000) + 2000);
            await processNetworkTasks(privateKey, networkConfigs.pharos, 'Pharos', userAddress, token, proxy, userAgent);
            await sleep(Math.floor(Math.random() * 3000) + 2000);
            await processNetworkTasks(privateKey, networkConfigs.monad, 'Monad', userAddress, token, proxy, userAgent);
            await initiateTransaction({ address: userAddress }, privateKey);

            const delay = Math.floor(Math.random() * (config.maxDelayBetweenWallets - config.minDelayBetweenWallets)) + config.minDelayBetweenWallets;
